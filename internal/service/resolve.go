...
func (s *Service) Resolve(ctx context.Context, req model.ResolveRequest) (model.ResolveResponse, error) {
	...
	defer lock.Unlock()
	now := clock()
	run := model.ResolveRun{ID: req.ID, RootComponent: req.RootComponent, RootConstraint: req.RootConstraint, Platform: req.Platform, AllowPrerelease: req.AllowPrerelease, Locks: req.Locks, Policy: req.Policy, InputDigest: solver.InputDigest(req), Status: model.RunPending, CreatedAt: now}
